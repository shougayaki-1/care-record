  ...
  const [{ data: shiftsWithLinks, error: shiftsError }, { data: reports, error: reportsError }, { data: premiumTypes, error: premiumTypesError }, internalWorkRecords] = await Promise.all([
    sessionClient
      ...
      .select(`
        ...
        report_shifts (is_primary, reports(id, start_at, end_at, status, segment_id, actual_service_type_id, deleted_at, report_values(data), actual_service_type:service_types(name), report_actual_staffs(staff:staffs(name)))),
        shift_segments (
          id, start_at, end_at, sort_order,
          service_type:service_types(name),
          shift_segment_staffs(staff_id, staff:staffs(name)),
          reports(id, start_at, end_at, status, segment_id, actual_service_type_id, deleted_at, report_values(data), actual_service_type:service_types(name), report_actual_staffs(staff:staffs(name)))
        )
      `)
    ...
    sessionClient
      ...
      .select(`
        ...
        report_values (data),
        actual_service_type:service_types(name),
        report_actual_staffs(staff:staffs(name)),
