...
export default function ClientSettingsPage() {
    ...
    const fetchClientData = async () => {
        try {
            ...
            if (template?.schema && Array.isArray(template.schema) && template.schema.length > 0) {
                setFormItems(template.schema as any);
            } else {
    ...
    const fetchOtherClients = async () => {
        ...
        if (!profile) return;
        const { data } = await supabase.from('clients').select('id, name').eq('organization_id', profile.organization_id).neq('id', clientId);
    ...
    const updateField = (index: number, key: keyof FormItem, value: any) => {
        const newItems = [...formItems];
                            ...
                            <Stack direction={{ xs: 'column', md: 'row' }} alignItems="flex-start" spacing={2}>
                                ...
                                <Box sx={{ flexGrow: 1, width: '100%' }}>
                                    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} mb={1}>
                                        <TextField
                                            select label="種類" size="small" value={item.type} onChange={(e) => updateField(index, 'type', e.target.value)} sx={{ minWidth: 160 }}
