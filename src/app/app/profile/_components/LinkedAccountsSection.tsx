...
interface Props {
    ...
    onLinkIdentity: (provider: 'google' | 'azure') => Promise<void>;
}

export function LinkedAccountsSection({ linkedProviders, onLinkIdentity }: Props) {
    return (
        <Stack spacing={2}>
                    ...
                    <Box key={p} display="flex" justifyContent="space-between" alignItems="center" p={2} border="1px solid" borderColor="divider" borderRadius={2}>
                        <Box display="flex" alignItems="center" gap={2}>
                            ...
                            <Typography fontWeight="bold" textTransform="capitalize">
                                {p === 'azure' ? 'Microsoft' : 'Google'}
                            </Typography>
                        </Box>
                        {linkedProviders.includes(p) ? (
                            <Chip label="連携済" color="success" size="small" icon={<CheckCircleIcon />} />
                        ) : (
                            <Button variant="outlined" size="small" onClick={() => onLinkIdentity(p as 'google' | 'azure')}>
                                連携
                            </Button>
                        )}
                    </Box>
