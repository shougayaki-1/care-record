    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 3 }}>
        <Container maxWidth="md" disableGutters>
            ...
            {isSpanningMonth && (
                <Paper variant="outlined" sx={{ p: 2, bgcolor: tokens.status.warning.bgFaint, borderColor: tokens.status.warning.highlight, borderRadius: 3 }}>
            ...
            {groupedSections.map((section, idx) => (
                ...
                <Box sx={{ bgcolor: tokens.neutral.bg, px: 3, py: 2, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center' }}>
                ...
                <Stack divider={<Divider />}>
                    ...
                    return (
                        <Box key={item.id} sx={{ p: 3, bgcolor: hasError ? tokens.status.error.bg : 'transparent' }}>
                        {item.type === 'checkbox' && (
                        ...
                        {item.type === 'multicheckbox' && (
                            ...
                            <FormGroup row sx={{ gap: 1 }}>
                                ...
                                <FormControlLabel key={opt} sx={{ mr: 2, mb: 1, border: '1px solid', borderRadius: 2, px: 1.5, py: 0.5, mx: 0, '&:hover': { bgcolor: tokens.neutral.gray100 }, bgcolor: ((answers[item.id] as string[]) || []).includes(opt.trim()) ? tokens.blueTint[100] : 'transparent', borderColor: ((answers[item.id] as string[]) || []).includes(opt.trim()) ? 'primary.main' : 'divider' }} control={<Checkbox size="small" checked={((answers[item.id] as string[]) || []).includes(opt.trim())} onChange={e => { const current = (answers[item.id] as string[]) || []; const next = e.target.checked ? [...current, opt.trim()] : current.filter((v: string) => v !== opt.trim()); handleAnswerChange(item.id, next); }} />} label={<Typography variant="body2" fontWeight={((answers[item.id] as string[]) || []).includes(opt.trim()) ? 'bold' : 'normal'}>{opt.trim()}</Typography>} />
                                ))}
                            </FormGroup>
