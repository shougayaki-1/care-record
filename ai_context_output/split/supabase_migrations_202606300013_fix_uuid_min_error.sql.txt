--- START OF FILE supabase/migrations/202606300013_fix_uuid_min_error.sql ---

-- 記録保存時のシフト自動紐付けロジック内で、PostgreSQLに存在しない min(uuid) を
-- 呼び出してしまっていたエラー（42883: function min(uuid) does not exist）を修正します。
-- count(*) = 1 の際に確実に1件を取得するため、min(id::text)::uuid にキャストして対応します。
...
REVOKE ALL ON FUNCTION public.save_report_atomic(uuid,uuid,uuid,uuid,uuid,timestamptz,timestamptz,text,jsonb,text,uuid,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.save_report_atomic(uuid,uuid,uuid,uuid,uuid,timestamptz,timestamptz,text,jsonb,text,uuid,jsonb) TO authenticated;

-- フロントエンドから直接呼び出されるラッパー関数も、同じく再定義して最新の関数のシグネチャと合わせます
...
--- END OF FILE supabase/migrations/202606300013_fix_uuid_min_error.sql ---