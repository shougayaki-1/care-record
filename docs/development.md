...
Node.js 24、npm、Docker を用意し、次の順でローカル環境を起動します。
...
日常の変更は `feature/*` から PR を作り、確認後 `main` に merge します。
常設 `staging` ブランチは必須ではありません。既存の Staging Supabase と
Vercel Preview プロジェクトは、ホスト済み Auth・Storage・連携動作が必要な
確認に残します。Production と Staging の値を共有しません。

アプリは Vercel Git 連携から配備する方針です。DB migration は手動で
Staging、Production の順に適用し、Production では適用前に最新 backup と
